...
import { h, fmt, fp, ff, clamp, tooltipFor, icon, svgIcon } from './util.js';
...
export function createInspector(root, { onWhy, onAction, onOpenTab, onClose }) {
  ...
  function globalPanel() {
    ...
    const head = h('div', { class: 'p-head' },
      h('div', { class: 'p-head-row' }, h('div', { class: 'p-title' }, h('span', { class: 'kicker' }, 'Patient'), h('h2', {}, 'Controls')),
  ...
  function render() {
    ...
    const els = !sel ? globalPanel() : sel.type === 'edge' ? edgePanel(sel.id) : sel.type === 'node' ? nodePanel(sel.id) : globalPanel();
