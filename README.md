...
- **Stage.** A focused plate of the portal circulation: the portal tree, the liver with its microcirculation (tinted by sinusoidal pressure), hepatic outflow to the right atrium, and the collaterals. Vessels that only close the systemic loop (aorta, limb and head vessels, the right kidney) stay in the model but aren't drawn; the IVC sits behind the liver as it does anatomically. The circuit view is a transit map in which pressure falls from left to right and collaterals loop around the outside. The anatomy fills the center. Pressure labels hang in the margins with thin leader lines, like an atlas plate, and route around anything floating over them. The playback bar (play, speed, the Seconds/Months clock, fast-forward, settle) sits in the top bar on wide screens and floats over the stage on narrow ones. The toolbar sits at the bottom, grouped as Inspect · Disease · Treat · Measure. The pressure legend is on the left, zoom on the right. Events appear as small notifications in the corner, with a pulse ring on the organ where they happened.
