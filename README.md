...
- **Findings.** Threshold events (a collateral opening, flow reversing, a varix rupturing, ascites forming) never cover the figure and never flash. They collect in the Findings tab of the side panel, which shows a count of unread items, and the list gives each one's detail and a **Why?** link. Active bleeding is a steady status in the header. Every event is also kept in the Log instrument.
